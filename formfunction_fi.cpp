...
PetscErrorCode FormFunctionLocal(DMDALocalInfo* info, PhysicalField** x,
    ...
    CHKERRQ(ierr);
         ...
         char filename[PETSC_MAX_PATH_LEN - 1];
            sprintf(filename, "userexample=%dpermeability_xxascii_%d.vts", EXAMPLE,
                    tsctx->tscurr);
            ierr = DataSaveVTK(perm_local, filename);
            sprintf(filename, "userexample=%dpermeability_xxascii_%d.data", EXAMPLE,
                    tsctx->tscurr);
            ierr = DataSaveASCII(perm_local, filename);
            CHKERRQ(ierr);
              }
#endif
#if 0
    ...
    CHKERRQ(ierr);
    ierr = DMGetNamedGlobalVector(secondary_dm, "sec_conc_old", &sec_conc_old);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(secondary_dm, eqm_k, &eqm_k_field);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(secondary_dm, sec_conc_old, &sec_conc_old_field);
    CHKERRQ(ierr);
    ierr = DMGetNamedGlobalVector(reaction_dm, "mass_frac_old", &mass_frac_old);
    CHKERRQ(ierr);
    ierr = DMGetNamedGlobalVector(reaction_dm, "initial_ref", &initial_ref);
    CHKERRQ(ierr);
    ierr =
        DMDAVecGetArrayRead(reaction_dm, mass_frac_old, &mass_frac_old_field);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(reaction_dm, initial_ref, &initial_ref_field);
    CHKERRQ(ierr);
...
#define RHO_OLD(i, j) (rho_init)
        ...
        for (i = xints; i < xinte; i++) {
            ...
            if (i == 0) {
                ...
                if (tsctx->tcurr < T_final) {
                    x_left.cw[1] =
                        2 * (5.e-2 +
                             (1.e-6 - 5.e-2) * (sin(0.5 * 3.14159 *
                                                    tsctx->tcurr / T_final))) -
                        x_center.cw[1];
                    x_left.cw[3] =
                        2 * (1.e-6 +
                             (5.e-2 - 1.e-6) * (sin(0.5 * 3.14159 *
                                                    tsctx->tcurr / T_final))) -
                        x_center.cw[3];
                } else {
            ...
            ReactionField _mass_frac_left, _mass_frac_right, _mass_frac_bottom,
                _mass_frac_top, _mass_frac, _mass_frac_old;
            SecondaryReactionField _sec_conc_left, _sec_conc_right,
                _sec_conc_bottom, _sec_conc_top, _sec_conc, _sec_conc_old;
            if ((tsctx->tcurr == 0)) {
                for (int r = 0; r < DOF_Secondary; ++r) {
                    _sec_conc_old.reaction_secondary[r] = 1.e-7;
                }

            } else {
                PorousFlowMassFractionAqueousEquilibriumChemistry_computeQpProperties(
                    &_sec_conc_old, &eqm_k_field[j][i], &_mass_frac_old,
                    &xold_field[j][i], _equilibrium_constants_as_log10, user,
                    tsctx->tcurr);
            }

            PorousFlowMassFractionAqueousEquilibriumChemistry_computeQpProperties(
                &_sec_conc_left, &eqm_k_field[j][i], &_mass_frac_left, &x_left,
                _equilibrium_constants_as_log10, user, tsctx->tcurr);

            PorousFlowMassFractionAqueousEquilibriumChemistry_computeQpProperties(
                &_sec_conc_right, &eqm_k_field[j][i], &_mass_frac_right,
                &x_right, _equilibrium_constants_as_log10, user, tsctx->tcurr);

            PorousFlowMassFractionAqueousEquilibriumChemistry_computeQpProperties(
                &_sec_conc, &eqm_k_field[j][i], &_mass_frac, &x_center,
                _equilibrium_constants_as_log10, user, tsctx->tcurr);

            PorousFlowMassFractionAqueousEquilibriumChemistry_computeQpProperties(
                &_sec_conc_bottom, &eqm_k_field[j][i], &_mass_frac_bottom,
                &x_bottom, _equilibrium_constants_as_log10, user, tsctx->tcurr);

            PorousFlowMassFractionAqueousEquilibriumChemistry_computeQpProperties(
                &_sec_conc_top, &eqm_k_field[j][i], &_mass_frac_top, &x_top,
                _equilibrium_constants_as_log10, user, tsctx->tcurr);
           ...
           // printf("....%g\n", sec_conc_old_field[j][i].reaction_secondary[0]);
#endif

            diff = 0.5 * (dx / (K1_xx(i, j)) + dx / (K1_xx(i, j)));
            U_L = -conc_1(i, j) * ((x_center.pw - x_left.pw)) / diff;

            diff = 0.5 * (dx / (K1_xx(i, j)) + dx / (K1_xx(i, j)));
            U_R = -conc_1(i, j) * ((x_right.pw - x_center.pw)) / diff;

            diff = 0.5 * (dy / (K1_yy(i, j)) + dy / (K1_yy(i, j)));
            U_B = -conc_1(i, j) * ((x_center.pw - x_bottom.pw)) / diff;

            diff = 0.5 * (dy / (K1_yy(i, j)) + dy / (K1_yy(i, j)));
            U_T = -conc_1(i, j) * ((x_top.pw - x_center.pw)) / diff;

            fluxL = rho(i, j) * max(U_L, 0.0) + rho(i, j) * min(U_L, 0.0);

            fluxR = rho(i, j) * min(U_R, 0.0) + rho(i, j) * max(U_R, 0.0);

            fluxB = rho(i, j) * max(U_B, 0.0) + rho(i, j) * min(U_B, 0.0);

            fluxT = rho(i, j) * min(U_T, 0.0) + rho(i, j) * max(U_T, 0.0);

            for (nc = 0; nc < DOF_reaction; ++nc) {
                fluxL1 =
                    _mass_frac_left.reaction[nc] * rho(i, j) * max(U_L, 0.0) +
                    _mass_frac.reaction[nc] * rho(i, j) * min(U_L, 0.0);
                fluxL2 =
                    rho(i, j) * diffusivity(i, j) *
                    (_mass_frac.reaction[nc] - _mass_frac_left.reaction[nc]) /
                    dx;
                fluxR1 =
                    _mass_frac_right.reaction[nc] * rho(i, j) * min(U_R, 0.0) +
                    _mass_frac.reaction[nc] * rho(i, j) * max(U_R, 0.0);
                fluxR2 =
                    rho(i, j) * diffusivity(i, j) *
                    (_mass_frac_right.reaction[nc] - _mass_frac.reaction[nc]) /
                    dx;
                fluxB1 =
                    _mass_frac_bottom.reaction[nc] * rho(i, j) * max(U_B, 0.0) +
                    _mass_frac.reaction[nc] * rho(i, j) * min(U_B, 0.0);
                fluxB2 =
                    rho(i, j) * diffusivity(i, j) *
                    (_mass_frac.reaction[nc] - _mass_frac_bottom.reaction[nc]) /
                    dy;
                fluxT1 =
                    _mass_frac_top.reaction[nc] * rho(i, j) * min(U_T, 0.0) +
                    _mass_frac.reaction[nc] * rho(i, j) * max(U_T, 0.0);
                fluxT2 =
                    rho(i, j) * diffusivity(i, j) *
                    (_mass_frac_top.reaction[nc] - _mass_frac.reaction[nc]) /
                    dy;
                alpha[nc] = (rho(i, j) * phi_field[j][i].xx[0] *
                                 _mass_frac.reaction[nc] -
                             RHO_OLD(i, j) * phi_old_field[j][i].xx[0] *
                                 _mass_frac_old.reaction[nc]) /
                            tsctx->tsize;
                ...
                f[j][i].cw[nc] =
                    scaling_1 *
                    (alpha[nc] + (fluxR1 - fluxL1) / dx +
                     (fluxT1 - fluxB1) / dy -
                     ((fluxR2 - fluxL2) / dx + (fluxT2 - fluxB2) / dy) +
                     qn[nc]);
            }
#if EXAMPLE == 1
    ...
    CHKERRQ(ierr);
    ierr =
        DMDAVecRestoreArrayRead(reaction_dm, initial_ref, &initial_ref_field);
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr =
        DMRestoreNamedGlobalVector(secondary_dm, "sec_conc_old", &sec_conc_old);
    CHKERRQ(ierr);
    ierr = DMRestoreNamedGlobalVector(reaction_dm, "mass_frac_old",
                                      &mass_frac_old);
    CHKERRQ(ierr);
    ierr = DMRestoreNamedGlobalVector(reaction_dm, "initial_ref", &initial_ref);
    CHKERRQ(ierr);
